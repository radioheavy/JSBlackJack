...
function gameOver()
{
    ...
    const gameOverMessage = `Oyun bitti! Final Skorun - <span class = 'badge'>${score}</span> Başla tuşuna basarak oyuna başlayabilirsin`
...
function chooseCard(card)
{
    if(canChooseCard())
    {
        ...
        setTimeout(() => {
            flipCards(false)
            updateStatusElement(currentGameStatusElem,"block", primaryColor,"Kart kozları ortada!")
...
function outputChoiceFeedBack(hit)
{
    if(hit)
    {
        updateStatusElement(currentGameStatusElem, "block", winColor, "Buldun!! - Eline Sağlık!! :)")
    }
    else
    {
        updateStatusElement(currentGameStatusElem, "block", loseColor, "Lütfen Tekrar Dene!! :(")
    }
}
...
function shuffleCards()
{
    ...
    function shuffle()
    {
        ...
        if(shuffleCount == 500)
        {
            ...
            dealCards()
            updateStatusElement(currentGameStatusElem, "block", primaryColor, "Lütfen Maça Ası olduğunu düşündüğün kartı seç!")
